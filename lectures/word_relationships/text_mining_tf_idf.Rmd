...
Notice how the common *non-contextual* words ("the", "to", "and", etc.) have high tf values but their idf and tf-idf values are 0.  Remember that $\mbox{idf}(t,D) = \log(\frac{N}{n_{t}})$ so these common terms that occur in every document will have an $\mbox{idf} = \log(\frac{7}{7}) = 0$ An since tf-idf is simply $\mbox{tf} \cdot \mbox{idf}$ this will also be zero.  
