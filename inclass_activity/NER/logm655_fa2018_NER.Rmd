...
Suppose we care about finding about people or objects which are mentioned in a particular section of text and how they relate to each other in the context of the text.  In the Harry Potter texts, we could attempt to gain insights about particular chapters or particular books when looking at which named entities are mentioned the most or when they are mentioned.  

In Bryan's project, he is looking at the last 25 years worth of State of the Union Addresses to see what trends exist across presidents and parties.  In addition to document summarization to geneerate a condensed version of each speech, we could further analyze the text to see who and what each president mentions, how often, and in what context.   
...
The example shown above blends NER, event coreference (which event mentioned in a given text are the same event), temporal expressions (days of the weeks, months), entity linking (United Airlines vs. United).  While this lesson only delves into NER, this example shows some of the things that could be extracted to answer a question like: "Is there any relationship between one airline changing fare price followed by others doing to same thing?"
...
CRF models are undirected graphical models, so it it’s not computing a probability for each entity at each time step. Instead, at each time step the CRF computes log-linear functions over a clique,
a set of relevant features. The undirected nature of CRFs allows them to also use future output features to predict a current entity where as MEMM models only can look at current and previous features. 
...
As CRF have to rely on gazetteers and lists, the are somewhat of a sub-prime solution as there are numerous flaws with these lists in terms of cost and inherent limitations.  NLP experts have begun to use recurrent neural networks to for NER because of their ability to handle sequential data. 

The neural algorithm normally used for NER is a bi-directional Long Short Term Memory (bi-LSTM) which divide the text into two portions: one in which the data is no longer needed and one which keeps information that will likely be used later.  Of the words it keeps, it maps each of the words into their word embeddings (which are vectors of real numbers).

The neural net them conducts the bi-directional LSTM layers which read each embedding and give an output after each hidden layer.  A dense layer then takes its weights and then maps to output of the bi-LSTM layers to a *n*-dimensional vector of logits and probabilities.  
...
# COnclusions
NER, must like part of speech, has many uses in and of itself but can also be used to filter documents and run further NLP task on.  
