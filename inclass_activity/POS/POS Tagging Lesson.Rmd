...
The HHM is based on modifying the Markov chain. Markov chains tell us something about the probabilities of sequences of random variables, *states*, each of which can take on values from some set. To predict the future, all we need to know is the current state. For example, to priduct tomorrow's weather, you could examine today's weather, but not yesterday's weather.
